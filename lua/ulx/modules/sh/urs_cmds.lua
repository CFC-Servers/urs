...
function ulx.restrict( ply, type, what, ... )
    ...
    if not groups[1] then return end
...
function ulx.unrestrict( ply, type, what, ... )
    ...
    if not URS.restrictions[type][what] then
        return ULib.tsayError( ply, what .." is not a restricted ".. type )
...
function ulx.loadoutremove( ply, group, ... )
    if not URS.loadouts[group] then return ULib.tsayError( ply, group .." does not have a loadout" ) end
    local weapons = {...}
    local removers = {}
    local removers2 = {}
    ...
    if weapons[1] == " * " then
        ...
        xgui.sendDataTable( {}, "URSLoadouts" )
        return ulx.fancyLogAdmin( ply, URS.cfg.echoCommands:GetBool(), "#A removed the loadout from #s", group )
    else
